scoreboard players operation @s s8 *= @s mtply_10
function animations:pegasus_light

scoreboard players operation @e[tag=stargate,tag=pegasus,tag=ring_light,tag=in_setup,distance=0..1] r = @s s8
tp @e[tag=stargate,tag=pegasus,tag=ring_light,tag=in_setup,distance=0..1] ~ ~ ~
scoreboard players set @e[tag=stargate,tag=pegasus,tag=ring_light,tag=in_setup,distance=0..1] p_rotate_dif 160
